...
The `research/paper-trace-baseline` branch is the discovery stage of the study. It preserves the runtime behavior of DeepSeek Harness `47f943859b` and adds only implementation observations, deterministic scenarios, expected-failure validation, CI wiring, and documentation. The [Cordis trace-baseline kit](https://github.com/Stool233/cordis/tree/48c4604005b80b4e4fd7706088f5b721a16ea8de/formal) remains the sole executable specification; this repository does not copy its TLA+ modules.
...
Runtime corrections live in the study's conformance stage on `research/paper-conformance`, where the same traces must pass. The trace-free logic and ordinary regression tests live in the upstream-proposal stage on `fix/paper-conformance`. This baseline branch does not change session persistence ordering or vendored Cordis lifecycle behavior, and its source-only hook creates no model-visible or product-user-visible output. The study portal reproduces this stage with `npm run reproduce:baseline` and the complete journey with `npm run reproduce:study`.
